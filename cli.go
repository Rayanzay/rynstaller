...
func main() {
	...
	if *versionFlag {
		fmt.Println("rynstaller Cli", buildinfo.InstallerTag, "("+buildinfo.InstallerGitHash+")")
	...
	if !SliceContainsFunc(switches, func(b *bool) bool { return *b }) {
		...
		go func() {
			...
			if IsSelfOutdated {
				...
				Log.Warn("To update, select the 'Update rynstaller' option to update, or run with --update-self")
			}
		}()
	...
	if install {
		...
		Log.Info("Downloading latest ryncord files...")
...
func HandleScuffedInstall() {
	...
	fmt.Println("Otherwise, ryncord will likely not work.")
}
